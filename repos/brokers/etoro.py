...
from repos.utils.shared import CASH_SCHEMA, INITIAL_COLS, get_isins_from_companies
...
class EtoroRepo:
    ...
    def __init__(self):
        ...
        logger.info("Looking up isins from underlying Etoro...")
        underlying_description_to_isin = get_isins_from_companies(etoro_df.filter(pl.col("underlying")!= "EUR")["underlying"].to_list())
        logger.info("Finished looking up isins from underlying Etoro.")
        
        etoro_portfolio_df: pl.DataFrame = etoro_df.with_columns(
            [
                ...
                pl.col("underlying").replace(underlying_description_to_isin).alias("ISIN")
